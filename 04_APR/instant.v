...
module CHIP ( clk_p_i, reset_n_i, data_a_i, data_b_i, inst_i, data_o ,valid_o);
  input [7:0] data_a_i;
  input [7:0] data_b_i;
  output  valid_o;
  output [11:0] data_o;
  ...
  Lk LK_in( .clk_p_i(i_clk_p_i), .reset_n_i(i_reset_n_i), .data_a_i(i_data_a_i), .data_b_i(i_data_b_i), .valid_o(i_valid_o), .data_o(i_data_o) );
  ...
  YA2GSD ipad_valid_o (.O(valid_o), .I(i_valid_o), .E(n_logic1), .E2(n_logic0), .E4(n_logic0), .E8(n_logic0), .SR(n_logic0));
